function start() {
  ...
  // ensure the player hasn't started moving
  moving = false;
}

function stop() {
  gameRunning = false;
  document.getElementById("bee1").style.display = "none";
}
...
function moveBear(e) {
  ...
  const KEYLEFT = 37;
  const KEYRIGHT = 39;
  // checks if the player has moved and sets the lastStringTime
  if (moving === false) {
    // take start time
...
function addBee() {
  var bee = new Bee(nbBees + 1); //create object and its IMG element
  bee.display(); //display the bee
  bees.push(bee); //add the bee object to the bees array
}
...
function updateBees() {
  ...
  if (hits.innerHTML == 100) {
    clearTimeout();
    if (gameRunning) window.alert("GAME OVER");
    stop();
  }
